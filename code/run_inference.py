...
for file in os.listdir(IMAGE_DIR):
    if file.endswith('.jpg'):
        ...
        for persona in personas:
            output_file_path = os.path.join(OUTPUT_DIR, file.removesuffix('.jpg') + '-' + persona + ".txt")
            print(f"Processing image: {image_path} | persona: {persona}")
            ...
            with open(output_file_path, 'w', encoding='utf-8') as out_file:
                ...
                for model in models:
                    ...
                    # BASELINE
                    out_file.write(">>> BASELINE RESPONSE <<<\n")
                    try:
                        zero_response: ChatResponse = chat(model=model, messages=zero_shot_message)
                        out_file.write(zero_response.message.content + "\n\n")
                    except Exception as e:
                        out_file.write(f"[ERROR during Baseline]: {e}\n\n")

                    # Ambigious Question Context
                    out_file.write(">>> FOCUS AMBIGUITY CONTEXT RESPONSE <<<\n")
                    try:
                        zero_response: ChatResponse = chat(model=model, messages=zero_shot_message)
                        out_file.write(zero_response.message.content + "\n\n")
                    except Exception as e:
                        out_file.write(f"[ERROR during Focus Question]: {e}\n\n")

                    # VizWiz Question Context!
                    out_file.write(">>> VIZWIZ QUESTION CONTEXT RESPONSE <<<\n")
                    try:
                        zero_response: ChatResponse = chat(model=model, messages=zero_shot_message)
                        out_file.write(zero_response.message.content + "\n\n")
                    except Exception as e:
                        out_file.write(f"[ERROR during VizWiz]: {e}\n\n")
